// // src/admin/Pages/PricingCategories.jsx
...
// const emptyForm = { name: "", description: "", order: 0, active: true };
...
//     try {
//       const res = await axios.get(`${ENV.BASE_URL}/admin/pricing-categories`, {
...
//     if (!window.confirm("Delete this pricing category? Tours using it must be updated first.")) return;
//     try {
//       await axios.delete(`${ENV.BASE_URL}/admin/pricing-categories/${id}`, {
//         headers: { Authorization: token() },
//       });
//       toast.success("Pricing category deleted");
//       fetchItems();
//     } catch (err) {
...
//     try {
//       if (editingId) {
//         await axios.put(`${ENV.BASE_URL}/admin/pricing-categories/${editingId}`, form, {
//           headers: { Authorization: token() },
//         });
//         toast.success("Pricing category updated");
//       } else {
//         await axios.post(`${ENV.BASE_URL}/admin/pricing-categories`, form, {
//           headers: { Authorization: token() },
//         });
//         toast.success("Pricing category created");
//       }
//       resetForm();
//       fetchItems();
//     } catch (err) {
...
//         <h1 className="text-2xl font-extrabold text-slate-900">Pricing Categories</h1>
//         <p className="text-slate-400 text-sm -mt-6">
//           Manage the pricing tiers used inside tour packages (e.g. "Deluxe", "Executive", "Luxury", "Premium Luxury").
//         </p>

//         <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 shadow-sm space-y-4">
//           <h2 className="font-bold text-slate-800">{editingId ? "Edit Category" : "New Category"}</h2>
//           <input value={form.name} onChange={set("name")} placeholder="Name (e.g. Deluxe)" className={inputCls} required />
...
//           {items.length === 0 ? (
//             <p className="p-6 text-slate-400 text-sm">No pricing categories yet.</p>
//           ) : (
//             items.map((item) => (
...
function ItemModal({ item, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const url = isEdit
        ? `${ENV.BASE_URL}/admin/pricing-categories/${item._id}`
        : `${ENV.BASE_URL}/admin/pricing-categories`;
      await axios[isEdit ? "put" : "post"](url, formData, {
        headers: { Authorization: token },
      });
      toast.success(isEdit ? "Pricing category updated!" : "Pricing category created!");
      onSaved();
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-xl font-bold text-slate-900">{isEdit ? "Edit Pricing Category" : "New Pricing Category"}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-1.5">
            ...
            <input value={formData.name} onChange={set("name")} className={inputCls} placeholder="e.g. Deluxe" required />
          </div>
...
// ── Card ────────────────────────────────────────────────────────────────────
function PricingCategoryCard({ item, onView, onEdit, onDelete }) {
  return (
    <div
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center" style={{ fontFamily: "'Outfit',sans-serif" }}>
      <div className="text-center">
        ...
        <p className="text-slate-500">Loading pricing categories…</p>
      </div>
    </div>
  );
      ...
      <div className="min-h-screen bg-slate-50 p-4 md:p-8" style={{ fontFamily: "'Outfit',sans-serif" }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Pricing Categories</h1>
              <p className="text-slate-400 mt-0.5 text-sm">Manage pricing tiers (Deluxe, Executive, Luxury, etc.)</p>
            </div>
            <button
          ...
          {filtered.length === 0 ? (
            <div className="bg-white rounded-3xl p-16 text-center shadow">
              ...
              <p className="text-slate-500 text-lg font-medium">No pricing categories found</p>
              <p className="text-slate-400 text-sm mt-1">Add your first pricing category to get started</p>
            </div>
          ) : (
