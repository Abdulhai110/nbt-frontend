...
function PricingOptionsBuilder({ options, onChange, pricingCategories }) {
  const addRow = () => {
    onChange([
      ...options,
      { category: "", hotelOptions: "", price: "", priceUnit: "per_couple" },
    ]);
  };
  ...
  const removeRow = (idx) => onChange(options.filter((_, i) => i !== idx));
        ...
        <div key={idx} className="border border-slate-200 rounded-2xl p-4 space-y-3 relative">
          ...
          <Field label="Hotel / Resort Options" hint="e.g. Snow Land Hotel / Hotel Himalaya / Qayyam Skardu">
            <input
              ...
              className={inputCls}
              placeholder="List the hotel/resort names for this tier"
            />
          </Field>
        </div>
      ))}
...
export default function TourForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setLocations(t.locations || []);
        setPricingOptions(
          (t.pricingOptions || []).map((opt) => ({
            category: opt.category?._id || opt.category || "",
  ...
  const handleSubmit = async (e) => {
    ...
    for (const opt of pricingOptions) {
      if (!opt.category || !opt.price) {
        toast.error("Each pricing tier needs a category and price");
        return;
      }
    }
    ...
    data.append("locations", JSON.stringify(locations));
    data.append("pricingOptions", JSON.stringify(pricingOptions));
              ...
              <Section title="Basic Information" subtitle="Core details about this tour package">
                <Field label="Tour Title" required>
                  <input
                    ...
                    onChange={set("title")}
                    className={inputCls}
                    placeholder="e.g. Honeymoon Trip to Skardu Valley"
                    required
                  />
                </Field>
