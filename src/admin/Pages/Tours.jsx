        ...
        {allImages.length > 0 && (
          <div className="relative h-72 md:h-96 rounded-t-3xl overflow-hidden bg-slate-100">
                  ...
                  {allImages.map((_, i) => (
                    <button
                      ...
                      style={{
                        ...
                        height: 8,
                        background:
                          i === imgIdx ? "#151D4A" : "rgba(255,255,255,0.6)",
                      }}
                    />
                  ))}
                </div>
            ...
            <div className="absolute top-4 left-4 flex gap-2">
              <span
                className={`px-3 py-1 text-xs font-semibold rounded-full ${tour.published ? "bg-emerald-500 text-white" : "bg-amber-400 text-amber-900"}`}
              >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 bg-white/80 hover:bg-white w-8 h-8 rounded-full flex items-center justify-center text-slate-600 shadow transition text-lg"
            >
              ×
            </button>
          </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
              {allImages.map((img, i) => (
                <img
                  ...
                  style={{
                    borderColor: i === imgIdx ? "#151D4A" : "transparent",
                  }}
                />
              ))}
            </div>
    ...
    >
      ...
      <div className="relative h-52 bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-3 right-3">
          <span
            className={`px-2.5 py-1 text-xs font-bold rounded-full ${tour.published ? "bg-emerald-500 text-white" : "bg-amber-400 text-amber-900"}`}
          >
            {tour.published ? "Live" : "Draft"}
          </span>
        </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-bold text-slate-900 text-lg leading-snug mb-1 line-clamp-2">
          {tour.title}
        </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((tour) => (
                <TourCard
                  ...
                  tour={{
                    ...tour,
                    coverImage: tour.coverImage?.url || tour.coverImage,
                  }}
                  onView={setViewTour}
