...
export default function Tours() {
  ...
  const [plans, setPlans] = useState([
    { id: 1, name: "Basic Plan", price: 9.99, duration: 1 },
    { id: 2, name: "Pro Plan", price: 19.99, duration: 6 },
    { id: 3, name: "Premium Plan", price: 29.99, duration: 12 },
  ]);
  ...
  const fetchPlans = async () => {
    try {
      ...
      const response = await axios.get(`${ENV.BASE_URL}/admin/plan/list`, {
        headers: {
          Authorization: token,
        },
      });
      // Update plans with API data if available
      setPlans(response.data?.plans || plans);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        {loading ? (
          <div className="p-6 flex items-center justify-center">
            ...
            <span className="ml-3 text-base sm:text-lg text-gray-600">Loading plans...</span>
          </div>
        ) : plans.length === 0 ? (
            ...
            <table className="min-w-full table-auto border-collapse">
              <thead className="bg-indigo-100 text-indigo-900 uppercase text-xs sm:text-sm font-semibold">
                <tr>
                  ...
                  <th className="px-4 sm:px-6 py-3 sm:py-4 text-center w-48">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 text-gray-700">
                {plans.map((plan, index) => (
                  ...
                  >
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-sm sm:text-base">{index + 1}</td>
