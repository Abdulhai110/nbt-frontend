import React from "react";
import { Link } from "react-router-dom";

function TourCard(props) {
  const { tourID, tourImage, tourTitle, tourPrice, index } = props;
  return (
    <div className="tour-box th-ani">
      <div className="tour-box_img global-img">
        <img src={`${tourImage}`} alt="" />
      </div>
      <div className="tour-content">
        <h3 className="box-title">
          <Link to={`/tour-details/${tourID}`}>
            {tourTitle ? tourTitle : "Pak Tour Package"}
          </Link>
        </h3>
        {/* <div className="tour-rating">
                    <div
                        className="star-rating"
                        role="img"
                        aria-label="Rated 5.00 out of 5"
                    >
                        <span style={{ width: "100%" }}>
                            Rated
                            <strong className="rating">5.00</strong> out of 5
                            based on <span className="rating">4.8</span>(4.8
                            Rating)
                        </span>
                    </div>
                    <Link
                        to="/tour-details"
                        className="woocommerce-review-link"
                    >
                        (<span className="count">4.8</span>
                        Rating)
                    </Link>
                </div> */}
        <h4 className="tour-box_price">
          <span className="currency">{tourPrice ? tourPrice : "20000"}</span>
          /Person
        </h4>
        <div className="tour-action">
          <span>
            <i className="fa-light fa-clock" />
            {index + 1} Days
          </span>
          <Link to={`/tour-details/${tourID}`} className="th-btn style4">
            Detail View
          </Link>
        </div>
      </div>
    </div>
  );
}

export default TourCard;
